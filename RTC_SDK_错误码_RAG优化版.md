...
错误码: 23  
含义: 该功能被禁止使用。请在声网控制台开启相关能力，或联系声网技术支持  
分类: 功能限制错误  
模块: RTC 引擎  
出现场景: 调用的能力在控制台未开通或被策略禁止时返回  
备注: 技术预览能力相关错误码可能出现于此
...
分类: RDT 错误  
模块: RDT（实时数据传输）  
出现场景: 对端尚未建立或未准备好接收 RDT 时进行操作返回  
备注: 技术预览
...
错误码: 253  
含义: RDT 命令流超过限制（单条不超过 256 字节，频率不超过约 100 次/秒）  
分类: RDT 错误  
模块: RDT（实时数据传输）  
出现场景: RDT 命令发送过大或过频时返回  
备注: 技术预览
...
错误码: 254  
含义: RDT 数据流超过限制（单条不超过 128 KB，速率不超过约 4 Mbps）  
分类: RDT 错误  
模块: RDT（实时数据传输）  
出现场景: RDT 数据包过大或发送速率过高时返回  
备注: 技术预览
